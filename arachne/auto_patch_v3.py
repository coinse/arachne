...
def where_to_fix_from_bl(
	...
	#print ("Nodes to ", len(nodes_to_lookat))
	...
	t2 = time.time()
	print ("Time for generating empty_graph for localiser: %f" % (t2 - t1))

	#if which != 'lfw_vgg':
	#	num_of_top_nodes = 0 
	#else:# lfw_vgg
	#	num_of_top_nodes = None # return all fronts
	if which == 'lfw_vgg':
		pareto_ret_all = True # retrurn all

	places_to_fix, front_lst = alocaliser.run(
		indices_to_wrong,
		#num_of_top_nodes = num_of_top_nodes, 
		from_where_to_fix_nw_down = (nodes_to_lookat, indices_to_cands_and_grads),
		pareto_ret_all = pareto_ret_all)
		
	print ("Overall", len(places_to_fix), len(front_lst), np.sum([len(vs) for vs in front_lst]))	
	if which == 'lfw_vgg':
		#places = [p for ps in front_lst for p in ps]
		places_to_fix = [p for ps in front_lst[:10] for p in ps]

	print ('Found results', len(input_data), indices_to_wrong)
	print (places_to_fix)
	print (len(places_to_fix), len(indices_to_wrong))
...
def patch(
	...
	t2 = time.time()
	print ("Time for generating an initial feed_dict: %f" % (t2 - t1))

	#if indices_to_correct is None or indices_to_selected_wrong is None:
	#if indices_to_target is None:
	from utils.data_util import split_into_wrong_and_correct
	t1 = time.time()
	print ("Start computing initial predictions")
	#print ("Tensor", predict_tensor_name, corr_predict_tensor_name)
	...
	sess.close()
	t2 = time.time()
	print ("Time taken for the computation of the initial prediction: %f" % (t2 - t1))
	print ("Corr", len(correct_predictions), np.sum(correct_predictions))
	indices_to_target = split_into_wrong_and_correct(correct_predictions)
	entire_indices_to_wrong = indices_to_target['wrong']
	print ("Pre", predef_indices_to_wrong)

	# check whether gien predef_indices_to_wrong to wrong is actually correct
	if predef_indices_to_wrong is not None:
		#print (len(set(predef_indices_to_wrong[0])), len(set(indices_to_target['wrong'])))
		diff = set(predef_indices_to_wrong) - set(indices_to_target['wrong'])
		assert len(diff) == 0, diff 
		indices_to_target['wrong'] = predef_indices_to_wrong
		print ("============", indices_to_target['wrong'], predef_indices_to_wrong)


	print ('====================Selecting input data to look at during the patching============================')
	# check for sampling
	# check for correctly misclassified ones
	...
	t2 = time.time()
	print ("Time taken for selecting the target: %f" % (t2 - t1))
	print ('Final Labels:', np.argmax(predictions, axis = 1)[indices_to_correct])
	# logging
	...
	print ("New indices to the target", len(new_indices_to_target))
	#import sys;sys.exit()
	...
	t2 = time.time()
	print ("Time for generating an empty graph for sampled data X and y: %f" % (t2 - t1))
	...
	if loc_method == 'gradient_loss': # here, top n is not the number of inpouts, arather it is the number of neural weights to fix
		...
		t2 = time.time()
		print ("Time for finding locations: %f" % (t2 - t1))
		...
		#print ("Number of sample: {}".format(num_random_sample))
	...
	if search_method == 'DE':
		...
		#print ("Number of places to fix: %d" % (len(places_to_fix)))
		#print (places_to_fix)
