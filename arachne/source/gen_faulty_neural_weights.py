...
def get_indices_to_grad_above_mean(path_to_keras_mdl, idx_to_tl, X):
	...
	grad = np.abs(
		compute_gradient_to_output(
			path_to_keras_mdl, idx_to_tl, X, on_weight = True, wo_reset = True, by_batch = True)) # this can be a list

	if not isinstance(grad, Iterable):
		mean_grad_v = np.mean(grad.flatten())
		indices_to_above_mean = list(zip(*np.where(grad > mean_grad_v)))
	else:
		#grad_combined = sum([_grad.flatten().tolist() for _grad in grad], [])
		#mean_grad_v = np.mean(grad_combined)
		#indices_to_above_mean = list(zip(*np.where(grad_combined > mean_grad_v)))
		from functools import reduce

		num_w = np.sum([reduce(np.multipy, _grad.shape) for _grad in grad])
		sum_w = np.sum([np.sum(_grad) for _grad in grad])
		mean_grad_v = sum_w/num_w
		indices_to_above_mean = []
		for idx, _grad in enumerate(grad):
			_indices = list(zip(*np.where(_grad.flatten() > mean_grad_v)))
			_indices = list(zip([idx]*len(_indices), _indices))
			indices_to_above_mean.extend(_indices)
...
def random_sample_weights(path_to_keras_mdl, X, indices_to_target, num_sample = 1):
	...
	for idx in sel_layers:
		...
		selected_neural_weights.append((idx, sel_idx)) # for lstm, sel_idx is (0 or 1, idx_to_w) => (idx_to_l, (0 or 1, idx_to_w))
...
if __name__ == "__main__":
	...
	train_data, test_data = data_util.load_data(
		args.which_data, args.datadir, is_input_2d = args.which_data == 'fashion_mnist')
	...
	if args.which_data == 'fashion_mnist':
		selected_neural_weights = random_sample_weights(
			args.model_path, target_data[0].reshape(target_data[0].shape[0],1,target_data[0].shape[-1]), 
			indices_to_target_layers, num_sample = num_sample)
	else:
		selected_neural_weights = random_sample_weights(
			args.model_path, target_data[0], 
			indices_to_target_layers, num_sample = num_sample)
	...
	## WE HAVE TO LOOK FROM HERE (DONE ABOVE)
