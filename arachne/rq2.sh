...
then
	...
	python3 main_rq2.py -datadir $datadir -which simple_fm -which_data $which_data -tensor_name_file data/tensor_names/tensor.lastLayer.names -loc_method $loc -patch_target_key ${patch_target_key}.${seed} -path_to_keras_model data/models/fmnist_simple.h5 -seed $seed -iter_num $iter_num -target_indices_file $indices_file -dest $dest/fm/$loc -patch_aggr $patch_aggr
else
