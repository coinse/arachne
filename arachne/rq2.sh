...
then
	...
	python3 main_rq2.py -datadir $datadir -which simple_fm -which_data $which_data -tensor_name_file data/tensor_names/tensor.lastLayer.names -loc_method $loc -patch_target_key $patch_target_key -path_to_keras_model data/models/fmnist_simple.h5 -seed $seed -iter_num 100 -target_indices_file $indices_file -dest $dest/rq2/fm/$loc -patch_aggr $patch_aggr
else
	indices_file="indices/cm/cifar10.misclf.indices.csv"
	python3 main_rq2.py -datadir $datadir -which simple_cm -which_data $which_data -tensor_name_file data/tensor_names/tensor.lastLayer.names -loc_method $loc -patch_target_key $patch_target_key -path_to_keras_model data/models/cifar_simple_90p.h5 -seed $seed -iter_num 100 -target_indices_file $indices_file -dest $dest/rq2/cm/$loc -patch_aggr $patch_aggr
fi
