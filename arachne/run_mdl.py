...
import numpy as np
from utils.run_utils import run_model
...
parser.add_argument("-new_mdl", "--path_to_new_model", type = str)
...
# get weights
pred_name = os.path.basename(args.model).replace("model", "pred")[:-4]
...
print ("=========================Train============================")
aft_pred_df_train = run_model(new_model, train_X, train_y, args.which_data)
combined_df = combine_init_aft_predcs(init_pred_df_train, aft_pred_df_train)
filename = os.path.join(args.dest, pred_name + ".train.csv")
combined_df.read_pickle(filename)
